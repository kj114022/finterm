...
pub fn render(
    ...
    render_header(f, main_chunks[0], provider_name, provider_icon, provider_color, items.len());
...
fn render_feed_list(f: &mut Frame, area: Rect, items: &[FeedItem], selected_idx: usize, accent: Color) {
    ...
    let list = List::new(list_items)
        .block(Block::default()
            .borders(Borders::ALL)
            .border_style(if true { Style::default().fg(accent) } else { Theme::style_border() })
            .title(" Feed "));
    
    f.render_widget(list, area);
}

fn render_feed_item(item: &FeedItem, is_selected: bool, width: usize, _accent: Color) -> ListItem<'static> {
    ...
    if let Some(author) = &item.author {
        line2_spans.push(Span::styled(format!(" by {}", author), Theme::style_author()));
    }
    
    line2_spans.push(Span::styled(format!(" {}", item.time_ago()), Theme::style_time()));
...
fn render_item_preview(f: &mut Frame, area: Rect, item: &FeedItem) {
    ...
    let title = Paragraph::new(Line::from(vec![
        Span::styled(&item.title, Theme::style_header()),
    ]))
    .wrap(Wrap { trim: true });
    ...
    if let Some(author) = &item.author {
        meta_spans.push(Span::styled(format!(" | {}", author), Theme::style_author()));
    }
    
    meta_spans.push(Span::styled(format!(" | {}", item.time_ago()), Theme::style_time()));
    
    if let Some(score) = item.metadata.score {
        meta_spans.push(Span::styled(format!(" | {} pts", score), Theme::style_score()));
    }
    
    if let Some(comments) = item.metadata.comments {
        meta_spans.push(Span::styled(format!(" | {} comments", comments), Theme::style_comments()));
    }
    ...
    // Content preview
    let content = item.summary.as_ref()
...
fn render_status_bar(f: &mut Frame, area: Rect, message: Option<&str>, loading: bool) {
    let status = if loading {
        ...
        message.unwrap_or("jk:Navigate Enter:Open c:Comments o:Browser ?:Help q:Quit").to_string()
    };
